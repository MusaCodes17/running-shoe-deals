...
**Snapshot date:** 2026-07-14 (after R4.6 New-Retailer Onboarding Agent — MAINTENANCE_PLAN I1 promoted to roadmap: `services/onboarding.py` + 4 MCP tools + `retailer_onboarding` prompt + REST parity + `scrape_health.needs_onboarding`; 21 tests; suite 374 → **395**. Prior same-day: Defect Block B — D1 FK pragma + `rotation.delete_owned_shoe()`; D3/D4/D5/D6. Prior: Defect Block A. **All of R2 + RA1.0–RA1.4 (code) + R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + R4.6 + Defect Block A + Defect Block B now shipped; RA1.5 cutover is the blocking human task.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 + RA1.4 (code) + R3.1 + R3.2 + R3.3 + R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + R4.6 + Defect Block A + Defect Block B are now shipped.** Suite: **395 passing**. RA1.5 cutover is the blocking human task; roadmap R4.2/R4.3 remain blocked on R3.5.
