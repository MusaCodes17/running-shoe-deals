...
**Snapshot date:** 2026-07-08 (after Phase 2 Session K — **R2.4 shoe-type controlled vocabulary**. `shoe_type` promoted from free strings to a backend-owned vocabulary (`app/utils/shoe_types.py`) served at `GET /api/shoe-types`, validated on write (422 on off-vocab), with the frontend copy deleted (`lib/shoeTypes.js` reduced to presentation-only). Migration `c9d0e1f2a3b4` normalized 9 legacy `owned_shoes` free-text values (E4-reconciled). Suite 133 → **141**. Prior: Session J **chat rate limiting** (E8, suite → 133); Session I **R2.3 indexed reads + watchlist service**; Session H **R2.7 T7–T8 + Training polish**; Sessions F/G **R2.7 T1–T6**; Session E **R2.2**; Session D **R2.1**. **R2.1–R2.4 + R2.7 + rate limiting done; next active R2 item is R2.5 scrape observability.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **R1 done; C1/M3 safety fixes done; R2.1 (security) + R2.2 (schema authority) shipped and now *live*; R2.7 (training depth) complete (T1–T8); R2.3 (indexed reads + watchlist service) + chat rate limiting (E8) + R2.4 (shoe-type vocabulary) shipped.** The active R2 thread is now R2.5–R2.6.

1. **R2.5 scrape observability** — persist per-retailer scrape runs/attempts (started/finished/count/error) written by the orchestrator; surface health + trend in Settings → Sync & Scraping. The substrate R4.1 scheduling + R4.5 watchdog require; forces the documented single-process-lock decision.
2. **R2.6 server-side chat/memory persistence** — move Son of Anton conversations + checkpoint state off localStorage into the backend (design_decisions C8, the last ⚠️ scheduled-to-change item; depends on R2.1, now done).
