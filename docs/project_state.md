...
**Snapshot date:** 2026-07-14 (after Defect Block A session — D8: sold-out deals now retired at qualification time + Shopify pessimistic `in_stock` default; D7: composite kids filter checks URL handles + youth-size variants excluded from price pool and `sizes_available`; D2 confirmed already fixed (bcdddc2). 29 new tests; suite 323 → **352**. Prior: R4.2 — Scrape Reliability. **All of R2 + RA1.0–RA1.4 (code) + R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + Defect Block A (D8 + D7) now shipped; RA1.5 cutover is the blocking human task.**)
...
| Test suite | ✅ **352 passing** (38 modules; Defect Block A: `test_kids_filter.py` new +26, `test_orchestrator.py` +3; R4.2: `test_scraper_retry.py` new +4, `test_scrape_history.py` +6; R4.4: `test_coupon_hunter.py` new +8; R4.1: `test_schedule.py` new +9; R3.6: `test_race_advisor.py` new +14; R3.3: `test_shoe_review.py` new +7; R3.2: `test_deal_alerts.py` new +22; R3.1: `test_weekly_summary.py` new +20; RA1.3: `test_access_log.py` new +15; `test_auth.py` +5; `test_oauth.py` +3; RA1.1b: `test_oauth.py` new +18; RA1.1: `test_auth.py` rewritten +8 net; earlier sessions: see prior entries; 1 pre-existing env-leak in `test_schedule.py` not counted in this total — that test passes in isolation) |
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 + RA1.4 (code) + R3.1 + R3.2 + R3.3 + R3.4 + R3.6 + R4.1 + R4.2 + R4.4 + Defect Block A (D8 + D7) are now shipped.** Suite: **352 passing**. RA1.5 cutover is the blocking human task; roadmap R4.2/R4.3 remain blocked on R3.5.
...
2. **Defect Block B (D1 → D3/D4 batch → D6)** — D1: FK pragma + sanctioned shoe delete; D3: COROS dedup verification; D4: batched hazard fixes (`trigger_scrape` dict-as-list, `is_already_logged` ISO string vs Date, promo sort TypeError, brand case sensitivity); D6: env-leak fix in `test_schedule.py`. See `MAINTENANCE_PLAN.md` §1 for detail.
...
*Maintenance note: this file describes 2026-07-14 (Defect Block A — D8 sold-out deals + D7 kids/youth filter) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
