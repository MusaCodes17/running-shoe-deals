...
**Snapshot date:** 2026-07-09 (after RA1.3 session — **Surface & abuse hardening**: `AccessLogMiddleware` (structured per-request log + credential redaction) + per-IP auth-failure rate limiter (429 after burst) + 401 logging with source IP + login-page rate limiter + Caddyfile comment update; suite 210 → **231** (+23 tests). Prior: RA1.1b — OAuth 2.1 connector auth. Prior: RA1.2 — Deployment substrate. Prior: RA1.0 + RA1.1 — research spikes, D0 hosting decision, named per-client tokens. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 are now shipped; RA is the active milestone; R3/R4 are ⏸ parked.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 are now shipped.** Suite: **231 passing**. RA is the active milestone; R3/R4 are ⏸ parked.

0. ~~**RA1.1b**~~ ✅ **Done (2026-07-09)** — OAuth 2.1 (`AntonOAuthProvider` + login page + 18 tests, capability-URL deleted). Suite 194 → 210.
0. ~~**RA1.3**~~ ✅ **Done (2026-07-09)** — Auth-failure 401 logging + per-IP rate limiter (429) + `AccessLogMiddleware` (structured per-request log + credential redaction) + login-page rate limiter + Caddyfile comment. Suite 210 → 231. **Remaining human step:** set up external uptime pinger on `/health` before RA1.5 cutover.
1. **RA1.4 — Backups off-laptop** — Litestream continuous replication (preferred) or nightly snapshot to runner-controlled object storage; restore drill performed before cutover counts as done. `REMOTE_ACCESS_PLAN.md` §6 RA1.4.
2. *(after RA1.3 + RA1.4)* **RA1.5 — Cutover & validation** — provision host, deploy container, E4 count reconciliation, re-point Claude Desktop, add claude.ai connector; two exit criteria: mobile sync E2E on cellular + DC-IP scrape comparison via R2.5 `scrape_runs`. **Also during RA1.5:** configure uptime pinger on `/health` (RA1.3 remaining acceptance criterion).
3. *(after RA1)* **R3.1 Weekly Rotation Summary Agent** — first proactive agent (roadmap R3, order 3.1 → 3.4 → 3.3 → 3.2 → 3.5 → 3.6).
...
5. **Optional follow-ons:** the shoe-major synchronous scrape doesn't yet emit R2.5 `scrape_runs` (deliberate deferral, D8); watchlist reduction still O(N) Python in `services/watchlist.py` (labelled, fine); `deals`/`dashboard` fat routers remain; surface chat 429 as client-side toast.
