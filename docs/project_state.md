...
**Snapshot date:** 2026-07-10 (after R4.2 session — Scrape Reliability: retry in `_fetch_with_requests` + watchdog (`_derive_watchdog_alert`, `retailers_needing_attention`) + `scrape_health` docstring + 10 tests; suite 313 → **323** (322 passing; 1 pre-existing env-leak in `test_schedule.py`). Prior: R4.4 — Coupon Hunting Agent; R4.1 — Scheduled scraping; R3.6 — Race-block training advisor; **All of R2 + RA1.0–RA1.4 (code) + R3.1–R3.4 + R3.6 + R4.1 + R4.2 + R4.4 are now shipped; RA1.5 cutover is the blocking human task; R4.4 (Coupon Hunting) and R4.2 (Scrape Reliability) done; next code task is R4.5 or R4.3.**)  
Note: the roadmap's R4.5 (Scraper watchdog) was implemented as part of this session's R4.2; R4.5 is now done. Roadmap R4.2 (Agent scheduling) remains blocked on R3.5.
...
| Test suite | ✅ **322 passing** (323 collected; 1 pre-existing env-leak in `test_schedule.py`), 37 modules (R4.2: `test_scraper_retry.py` new +4, `test_scrape_history.py` +6; R4.4: `test_coupon_hunter.py` new +8; R4.1: `test_schedule.py` new +9; R3.6: `test_race_advisor.py` new +14; R3.3: `test_shoe_review.py` new +7; R3.2: `test_deal_alerts.py` new +22 + 2-test discrepancy from R3.4 resolved; R3.4: stable at 251; R3.1: `test_weekly_summary.py` new +20; RA1.4: stable; RA1.3: `test_access_log.py` new +15; `test_auth.py` +5; `test_oauth.py` +3; net +23; RA1.1b: `test_oauth.py` new +18; `test_auth.py` −4 capability-URL tests; net +14; RA1.2: stable; RA1.1: `test_auth.py` rewritten +8 net; R2.7.2: `test_races.py` +2; Session Q: +3; Session O: +13; Session N: +15; Session M: +9; Session L: +8; Session K: +8; Session J: +5; earlier sessions: see prior entries) |
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 + RA1.4 (code) + R3.1 + R3.2 + R3.3 + R3.4 + R3.6 + R4.1 + R4.2 (Scrape Reliability / R4.5 watchdog) + R4.4 are now shipped.** Suite: **322 passing** (323 collected). RA1.5 cutover is the blocking human task; roadmap R4.2/R4.3 remain blocked on R3.5.
...
2. ~~**R3.5 — Notification channel**~~ ⏸ **Deferred** — pull-based MCP tools (R3.1/R3.2/R3.6) already cover the on-demand surface; push delivery revisit trigger: when R4.2/R4.3/R4.5 surface a felt need for unattended delivery. See roadmap R3.5 deferral note.
3. **Optional follow-ons:** the shoe-major synchronous scrape doesn't yet emit R2.5 `scrape_runs` (deliberate deferral, D8); watchlist reduction still O(N) Python in `services/watchlist.py` (labelled, fine). ~~`deals`/`dashboard` fat routers remain~~ ✅ Done 2026-07-10. ~~surface chat 429 as client-side toast~~ ✅ Done 2026-07-10.

---

*Maintenance note: this file describes 2026-07-10 (R4.2 Scrape Reliability + R4.4 Coupon Hunting Agent) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
