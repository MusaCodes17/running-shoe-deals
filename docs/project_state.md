...
**Snapshot date:** 2026-07-10 (after R3.2 session — Deal Alert Agent: `services/deal_alerts.py` + 22 tests + `get_deal_alerts` MCP tool + `deal_alert_digest` MCP prompt; suite 251 → **275** (2-test discrepancy from R3.4 resolved). Prior: R3.4 — MCP watchlist parity + resource expansion; prior: R3.1 — Weekly Rotation Summary Agent; prior: debt-sweep — fat router extraction + chat 429 toast; suite stable at **231**. Prior: RA1.4 — Backups off-laptop; RA1.3 — auth hardening; RA1.1b — OAuth 2.1; RA1.2 — deployment substrate; RA1.0/RA1.1 — research + named-token auth. **All of R2 + RA1.0–RA1.4 (code) + R3.1 + R3.2 + R3.4 are now shipped; RA1.5 cutover is next; R3 continues after.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 + RA1.4 (code) + R3.1 + R3.2 + R3.4 are now shipped.** Suite: **275 passing**. RA1.5 cutover is the blocking human task; R3 continues in parallel.
...
2. **R3.3 — Shoe review pipeline maturation** — next in R3 order; `draft_shoe_review` (MCP sampling) grows into a retirement-triggered workflow with stored draft.
...
*Maintenance note: this file describes 2026-07-10 (R3.2 Deal Alert Agent) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
