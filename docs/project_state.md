...
**Snapshot date:** 2026-07-10 (after R3.4 session — MCP watchlist parity + resource expansion: `get_watchlist` tool + `deals://watchlist`, `training://summary`, `training://fitness` resources; suite stable at **251**. Prior: R3.1 — Weekly Rotation Summary Agent (`services/weekly_summary.py` + `get_weekly_summary` MCP tool + `weekly_rotation_summary` MCP prompt; suite 231→253, but live count is 251 — 2-test discrepancy from R3.1's "previously masked" note). Prior: debt-sweep — fat router extraction + chat 429 toast; suite stable at **231**. Prior: RA1.4 — Backups off-laptop; RA1.3 — auth hardening; RA1.1b — OAuth 2.1; RA1.2 — deployment substrate; RA1.0/RA1.1 — research + named-token auth. **All of R2 + RA1.0–RA1.4 (code) + R3.1 + R3.4 are now shipped; RA1.5 cutover is next; R3 continues after.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.1b + RA1.2 + RA1.3 + RA1.4 (code) + R3.1 are now shipped.** Suite: **253 passing**. RA1.5 cutover is the blocking human task; R3 continues in parallel.
...
0. ~~**R3.1 Weekly Rotation Summary Agent**~~ ✅ **Done (2026-07-10)** — `services/weekly_summary.py` + `get_weekly_summary` MCP tool + `weekly_rotation_summary` MCP prompt. Suite 231 → 253 (live 251). Backend restart needed to pick up the new service import.
0. ~~**R3.4 — MCP watchlist parity**~~ ✅ **Done (2026-07-10)** — `get_watchlist` MCP tool + `deals://watchlist` resource + `training://summary` resource + `training://fitness` resource. Suite stable at 251. Backend restart needed to pick up the changes.
...
2. **R3.2 — Deal Alert Agent** — detection + digest for deal events (new deal on tracked shoe, price-drop, replacement-deal for pipeline shoe). Next in R3 order after R3.4.
...
*Maintenance note: this file describes 2026-07-10 (R3.4 MCP watchlist parity + resource expansion) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
