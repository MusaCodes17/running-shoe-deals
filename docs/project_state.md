...
**Snapshot date:** 2026-07-09 (after RA1.2 session — **Deployment substrate**: `backend/Dockerfile` (Python 3.11-slim + Playwright/Chromium + `--workers 1`), `backend/.dockerignore`, `docker-compose.yml`, `deploy/Caddyfile` (TLS + `flush_interval -1` unbuffered streaming + credential-redacting log filter), `deploy/.env.production.example`, **INV-9** in CLAUDE.md §14 (single-worker invariant). Suite stable at **194** (no code changes — "196" in previous entry was a 2-count doc drift). RA1.2 → ✅. Next: RA1.1b (OAuth 2.1 decision gate, half-day timebox) + RA1.3 hardening. Prior: RA1.0 + RA1.1 session — **RA1.0 research spikes (S1–S3) + D0 hosting decision; RA1.1 auth v2** (named per-client tokens + capability-URL; `ANTON_SECRET` rotated). Also **R2.7.2** races. Suite 188 → **194** (auth rewrite net). **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 + RA1.0 + RA1.1 + RA1.2 are now shipped; RA is the active milestone; R3/R4 are ⏸ parked.**)
...
| Test suite | ✅ **194 passing**, 27 modules (RA1.2: stable — no code changes; RA1.1: `test_auth.py` rewritten +8 net (named-token map, capability-URL, wrong-URL rejected); R2.7.2: `test_races.py` +2 (activity-tagged synthesis, dedup); Session Q: `test_rotation.py` +1, `test_fitness.py` +2 — R2.7.1 F1+F3; Session O: H2 orphan-guard fix flips xfail + `test_http_smoke.py` +12 — HTTP-layer serialization; Session N: `test_deal_store.py` +9, `test_orchestrator.py` +6; Session M: `test_chat_history.py` +6, `test_checkpoints.py` +3; Session L: `test_scrape_history.py` +8; Session K: `test_shoe_types.py` +8; Session J: `test_rate_limit.py` +5; Session I: `test_activities_union.py` +1; Session H: `test_races.py` +2, `test_activity_tags.py` +18, `test_activities_union.py` +1; Session G: `test_fitness.py` +2, `test_activity_edit.py` +6, `test_activities_union.py` +1; Session F: `test_activity_tags.py` +3, `test_activities_model.py`/`test_activities_union.py` +2/+4; `test_auth.py` +13 Session D) |
...
| RA1 Auth v2 (RA1.0 + RA1.1) | ✅ **Shipped** (2026-07-09) — S1–S3 spikes closed; D0 = cloud VM; named per-client `ANTON_TOKENS`; capability-URL connector path; `ANTON_SECRET` rotated; E7 → E9. Suite 188 → 194. |
...
-7. **E9 — named per-client bearer tokens + capability-URL connector auth** (2026-07-09, RA1.1 — E7 → 🔁 Superseded). `ANTON_SECRET` replaced by `ANTON_TOKENS="name:token,..."` (desktop / loopback / spa each revocable independently); constant-time multi-token comparison without short-circuiting; capability-URL path `/mcp/<CONNECTOR_TOKEN>/...` lets the claude.ai connector authenticate without OAuth (path-rewritten in ASGI middleware, no bearer needed in the connector config field). `get_named_token("loopback")` replaces `ANTON_SECRET` in `chat_service`. Old secret rotated unconditionally (was baked into SPA bundles). Explicitly interim for the connector (acceptable under TLS + rate limiting + logging — RA1.3; full OAuth deferred).
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 + RA1.0 + RA1.1 + RA1.2 are now shipped.** Suite: **194 passing**. RA is the active milestone; R3/R4 are ⏸ parked.

0. ~~**[FOLLOW-UP] Live browser visual pass on R2.6**~~ ✅ Done (Session P). ~~**Deal-domain test gaps + H1/H2**~~ ✅ Closed (Sessions N–O). ~~**R2.7.1**~~ ✅ Done (Session Q). ~~**RA1.0 + RA1.1**~~ ✅ Done (2026-07-09). ~~**RA1.2**~~ ✅ **Done (2026-07-09)** — Dockerfile + Caddy + docker-compose + INV-9. Suite stable at 194.
2b. **RA1.1b — OAuth 2.1 decision gate (half-day timebox, before RA1.5).** The decision rule: do the S3 targeted code read + an OAuth skeleton attempt. If `mcp[cli]` 1.28 exposes OAuth authorization-server plumbing (provider interface + `/authorize`, `/token`, metadata routes) → build Path 1 (OAuth) and delete the capability-URL path; it never goes public. If hand-rolled from scratch → the shipped capability-URL goes public (RA1.3's log redaction is already in the Caddyfile as a hard precondition). See `REMOTE_ACCESS_PLAN.md` §6 RA1.1 for the full decision tree. **Must decide before RA1.5.**
2c. **RA1.3 — Surface & abuse hardening** — 401 logging with source IP, in-process or firewall-level auth-failure throttle, structured access log; credential redaction in all logs (Caddyfile log filter is written — verify it covers all paths once deployed); uptime monitoring on `/health`. `REMOTE_ACCESS_PLAN.md` §6 RA1.3.
2d. **RA1.4 — Backups off-laptop** — Litestream continuous replication (preferred) or nightly snapshot to runner-controlled object storage; restore drill performed before cutover counts as done. `REMOTE_ACCESS_PLAN.md` §6 RA1.4.
2e. *(after RA1.1b + RA1.3 + RA1.4)* **RA1.5 — Cutover & validation** — provision host, deploy container, E4 count reconciliation, re-point Claude Desktop, add claude.ai connector; two exit criteria: mobile sync E2E on cellular + DC-IP scrape comparison via R2.5 `scrape_runs`.
2f. *(after RA1)* **R3.1 Weekly Rotation Summary Agent** — first proactive agent (roadmap R3, order 3.1 → 3.4 → 3.3 → 3.2 → 3.5 → 3.6).
...
4. **Optional follow-ons:** the shoe-major synchronous scrape (`scrape_all_shoes` / single-shoe / MCP `trigger_scrape` sans shoe_id) doesn't yet emit R2.5 `scrape_runs` — instrument it if that path stays in use (deliberate deferral, D8); the watchlist reduction still reduces in Python inside `services/watchlist.py` (labelled O(N), fine); `deals`/`dashboard` fat routers remain to be thinned like watchlist was; surface the chat 429 as a client-side toast; consider throttling `POST /api/chat/resource/read` too.

---

*Maintenance note: this file describes 2026-07-09 (RA1.2 session) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
