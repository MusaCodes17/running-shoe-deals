...
**Snapshot date:** 2026-07-10 (after debt-sweep session — fat router extraction + chat 429 toast; suite stable at **231**. Prior: RA1.4 session — **Backups off-laptop**: Litestream continuous replication config + `entrypoint.sh` (restore-on-start + replicate-while-running) + Dockerfile Litestream install + `deploy/restore.sh` (restore drill) + `deploy/pull-snapshot.sh`. Suite stable at **231**. Prior: RA1.3 — auth hardening; RA1.1b — OAuth 2.1; RA1.2 — deployment substrate; RA1.0/RA1.1 — research + named-token auth. **All of R2 + RA1.0–RA1.4 (code) are now shipped; RA is the active milestone; R3/R4 are ⏸ parked.**)
...
*Maintenance note: this file describes 2026-07-10 (debt-sweep session — fat router extraction + chat 429 toast) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
