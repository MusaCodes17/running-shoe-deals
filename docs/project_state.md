...
**Snapshot date:** 2026-07-09 (after RA1.0 + RA1.1 session — **RA1.0 research spikes (S1–S3) + D0 hosting decision; RA1.1 auth v2** (named per-client tokens + capability-URL; `ANTON_SECRET` rotated; `ANTON_TOKENS` + `ANTON_CONNECTOR_TOKEN`). Also **R2.7.2**: activity-tagged past races auto-surface in Races card + View-all dialogs. Suite 188 → **196** (+8 auth tests, +2 races tests). RA1.0 + RA1.1 → ✅. Next: RA1.2 deployment substrate. Prior: Session Q — **R2.7.1 training depth follow-ups** (F1–F4): rich COROS fields wired through `log_run_to_shoe` + `sync_coros_runs` prompt, rolling-365-day volume tile, `running_level` + `sync_fitness` prompt end-to-end, Training tab 2×2 card grid. Migration `f2a3b4c5d6e7`. Suite 185 → **188**. R2.7.1 → ✅. Prior: Session P — **R2.6 live browser visual pass**; Session O — **H2 orphan-guard fix + H1 HTTP-layer smoke tests** (suite → **185**); Sessions N/M/L/K/J/I/H/F–G/E/D — all of R2. **All of R2 (2.1–2.7) + R2.7.1 + R2.7.2 are now shipped; RA is the active milestone; R3/R4 are ⏸ parked.**)
...
Ordered; "immediate" means *next few sessions*, not emergencies — nothing is on fire. **All of R2 (2.1–2.7) + R2.7.1 are now shipped** — R1 + C1/M3 safety fixes, R2.1 (security) + R2.2 (schema authority) live, R2.7 (training depth, T1–T8), R2.3 (indexed reads) + rate limiting (E8) + R2.4 (shoe-type vocabulary) + R2.5 (scrape observability) + **R2.6 (chat/memory persistence)** + **R2.7.1 (training follow-ups: rich fields, volume tile, fitness E2E, 2×2 grid)**. The ⚠️ scheduled-to-change decision list is empty. The deal-domain test gaps are now closed (Session N). **Reprioritized 2026-07-09: next is RA — Remote Access & Deployment (roadmap section RA; `REMOTE_ACCESS_PLAN.md` is the executing contract), starting with RA1.0 (hosting decision D0 + spikes S1–S3). R3 and R4 are ⏸ parked until RA1 ships** — the goal is COROS sync from Claude mobile anywhere, which requires Anton's MCP publicly reachable over HTTPS on an always-on host (connectors call from Anthropic's cloud, and the laptop sleeps). When RA1 lands, resume at R3.1 as previously planned.
...
2. ~~**RA1.0 + RA1.1**~~ ✅ **Done (2026-07-09)** — S1–S3 spikes closed; D0 = cloud VM; RA1.1 auth v2 shipped (named per-client tokens + capability-URL; suite 188 → 196; E7 → E9). R2.7.2 committed alongside (activity-tagged race synthesis).
2a. **RA1.2 — deployment substrate** — Containerize (Python 3.11 + Playwright, pins intact), local-disk volume for `~/anton-data`, TLS (platform/Caddy) with **unbuffered streaming verified** (chat SSE, scrape SSE, `/mcp` Streamable HTTP), startup `alembic upgrade head`, **one Uvicorn worker pinned** (D4 + E8 are in-process). `REMOTE_ACCESS_PLAN.md` §6 is the sequencing reference; RA1.3+RA1.4 follow.
2b. *(after RA1)* **R3.1 Weekly Rotation Summary Agent** — first proactive agent (roadmap R3, order 3.1 → 3.4 → 3.3 → 3.2 → 3.5 → 3.6).
