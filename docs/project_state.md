...
**Snapshot date:** 2026-07-10 (after R3.1 session — Weekly Rotation Summary Agent: `services/weekly_summary.py` + `get_weekly_summary` MCP tool + `weekly_rotation_summary` MCP prompt; suite **231 → 253** (+20 new tests). Prior: debt-sweep — fat router extraction + chat 429 toast; suite stable at **231**. Prior: RA1.4 session — **Backups off-laptop**: Litestream continuous replication config + `entrypoint.sh` (restore-on-start + replicate-while-running) + Dockerfile Litestream install + `deploy/restore.sh` (restore drill) + `deploy/pull-snapshot.sh`. Suite stable at **231**. Prior: RA1.3 — auth hardening; RA1.1b — OAuth 2.1; RA1.2 — deployment substrate; RA1.0/RA1.1 — research + named-token auth. **All of R2 + RA1.0–RA1.4 (code) + R3.1 are now shipped; RA1.5 cutover is next; R3 continues after.**)
...
**AI layer**
- MCP server: ~21 tools, 7 resources (markdown+JSON), `sync_coros_runs` prompt, `sync_fitness` prompt, `weekly_rotation_summary` prompt (R3.1), sampling-powered `draft_shoe_review`; mounted at `/mcp` with lifespan-merged session manager.
...
0. ~~**R3.1 Weekly Rotation Summary Agent**~~ ✅ **Done (2026-07-10)** — `services/weekly_summary.py` + `get_weekly_summary` MCP tool + `weekly_rotation_summary` MCP prompt. Suite 231 → 253. Backend restart needed to pick up the new service import.
...
2. **R3.4 — MCP watchlist parity** — expose the watchlist through MCP (tool + resource) once R2.3 extracted the service (done); add `training://fitness` resource (R2.7 T5). Next in R3 order after R3.1.
...
*Maintenance note: this file describes 2026-07-10 (R3.1 Weekly Rotation Summary Agent) and decays fastest of all the docs. Update the Snapshot date, §2 table, §9, and §11 at session end; move shipped items from §4/§5 into §3. When in doubt, the `docs/changelog.md` top entries are the source of truth for what happened; this file is the source of truth for what it means.*
