...
**Chosen:** The platform is **Anton** (assistant: **Son of Anton**) while the **repo name, folder path, and DB filename** retain "running-shoe-deals." (The **in-code product strings** — FastAPI title, root message, chat `SYSTEM_PROMPT`, the COROS-sync agent prompt, the MCP server name, the READMEs, and `package.json` — were flipped to **Anton** in R1, 2026-07-14.)
**Why:** Renaming the repo/folder/DB filename mid-flight is churn with near-zero user value and real risk (the Litestream replica path keys off the DB filename); the in-code strings had no such coupling and one is user-visible (the assistant).
**Advantages:** Zero breakage; identity now reads "Anton" everywhere a human or connector sees it, while the load-bearing path names stay stable.
**Trade-offs:** Mild permanent confusion for newcomers (including AI sessions) that the repo/folder still say "running-shoe-deals" — mitigated by the glossary (`domain_model.md` §7.1).
**Verdict:** 🔁 **Partially superseded — R1 in-repo strings executed 2026-07-14** (`MAINTENANCE_PLAN.md` §4 R1). The remaining retained names (repo, folder, DB filename) stay until the GitHub + local-folder rename (R2/R3) makes it free — targeted at/around the RA1.5 cutover so the deployed host and connector URL are born with the final name.
