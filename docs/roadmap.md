...
| RA1.1b | **OAuth 2.1 upgrade for the connector (pre-public gate)** | The shipped capability-URL (RA1.1) is demoted to implemented fallback. Before anything goes public, run the plan's timeboxed decision rule (half a day: S3 code read + OAuth skeleton). SDK plumbing exists (in 1.28 or a contained, A7-verified `mcp` bump) → build single-user OAuth 2.1 (static client, claude.ai callback allowlist, PKCE S256, one-password login, hashed tokens in SQLite) and **delete the capability-URL path** — it never goes public. Hand-rolled otherwise → the capability-URL goes public with RA1.3's log redaction as a hard precondition + a dated E9 revisit note. See plan §6 RA1.1. | The URL-as-credential leaks into every log line (app access log + Caddy path logging), and the connector is the *primary* long-term consumer — build its real auth once if the SDK makes it contained. But hand-rolled OAuth on the public internet is worse than an honest interim, hence the gate rather than a mandate. | RA1.1; **decide before RA1.5** | Medium–High (OAuth) / Low (keep fallback + redaction) |
