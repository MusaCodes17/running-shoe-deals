...
| **P1-4** | Deal domain **nearly untested** *(narrowed 2026-07-06: `test_deals.py` pins MSRP savings/refresh/no-MSRP→no-deal)* — retirement/requalification (4.2) and promo manual-beats-scraped (D6) still uncovered *(narrowed further 2026-07-14: D8 added OOS/orphan-guard coverage in `test_orchestrator.py`)*: much of the domain constitution enforced by convention plus nothing. Compounding: no HTTP-layer tests anywhere. *(The migrations-never-exercised sub-gap is resolved — see §8.3, T4.)* | Missing tests (§8.1) | **Active** (shrinking) | refactor.md **H1** |
